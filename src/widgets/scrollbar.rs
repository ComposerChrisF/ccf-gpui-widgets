...
//!
//! This module provides an internal scrollbar widget used by the [`Scrollable`](super::scrollable::Scrollable) component.
//! It handles the rendering and interaction of scrollbar thumbs and tracks.
...
impl ScrollbarAxis {
    /// Check if this is vertical only
    pub fn is_vertical(&self) -> bool {
        matches!(self, Self::Vertical)
    }

    /// Check if this is horizontal only
    pub fn is_horizontal(&self) -> bool {
        matches!(self, Self::Horizontal)
    }

    /// Check if this is both
    pub fn is_both(&self) -> bool {
        matches!(self, Self::Both)
    }

    /// Check if this includes vertical scrolling
    #[inline]
    pub fn has_vertical(&self) -> bool {
        matches!(self, Self::Vertical | Self::Both)
    }

    /// Check if this includes horizontal scrolling
    #[inline]
...
/// Shared state for scrollbar interaction
#[derive(Debug, Clone)]
pub struct ScrollbarState(pub(crate) Rc<Cell<ScrollbarStateInner>>);

/// Internal scrollbar state
#[derive(Debug, Clone, Copy)]
pub struct ScrollbarStateInner {
    pub(crate) hovered_on_thumb: Option<Axis>,
    pub(crate) dragged_axis: Option<Axis>,
    pub(crate) drag_pos: Point<Pixels>,
...
impl ScrollbarState {
    /// Initialize scrollbar as visible (resets fade timer)
    #[allow(dead_code)]
...
/// Scrollbar element for rendering scrollbar UI
...
impl Scrollbar {
    ...
    /// Set the scrollbar axis (builder pattern)
    #[must_use]
    #[allow(dead_code)]
    ...
    /// Position horizontal scrollbar at top instead of bottom (builder pattern)
    #[must_use]
    #[allow(dead_code)]
    ...
    /// Set the scroll content size (builder pattern)
    #[must_use]
    #[allow(dead_code)]
    ...
    fn get_thumb_color(&self, cx: &App) -> Hsla {
        let theme = get_theme_or(cx, self.custom_theme.as_ref());
        gpui::rgb(theme.text_muted).into()
    }

    fn get_track_color(&self, cx: &App) -> Hsla {
        let theme = get_theme_or(cx, self.custom_theme.as_ref());
        let mut color: Hsla = gpui::rgb(theme.bg_input).into();
        color.a = 0.3;
        color
    }

    fn get_hover_thumb_color(&self, cx: &App) -> Hsla {
        let theme = get_theme_or(cx, self.custom_theme.as_ref());
        let mut color: Hsla = gpui::rgb(theme.text_muted).into();
        color.a = 0.8;
        color
    }
}
...
/// Prepaint state for a single axis scrollbar
...
/// Prepaint state for the scrollbar element
...
impl Element for Scrollbar {
    ...
    fn prepaint(
        ...
        for axis in self.axis.all().into_iter() {
            ...
            let (thumb_bg, track_bg, _thumb_width, inset, radius) =
                if is_dragged || is_hovered_on_thumb {
                    (
                        self.get_hover_thumb_color(cx),
                        self.get_track_color(cx),
                        THUMB_ACTIVE_WIDTH,
                        THUMB_ACTIVE_INSET,
                        THUMB_ACTIVE_RADIUS,
                    )
                ...
                for state in prepaint.states.iter() {
                    ...
                    window.on_mouse_event({
                        ...
                        move |event: &MouseMoveEvent, _phase, _hitbox, cx| {
                            ...
                            } else if state.0.get().hovered_on_thumb == Some(axis) {
